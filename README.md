...
- 2024/6/11
- 新增示例 typeorm框架
- app-config.js文件新增 typeorm配置选项
- 新增 entities目录，用于存放 typeorm实体类 配置
- 新增全局插件 typeorm-db.js 用于注册框架，全局使用 fastify.orm 操作

- -----------------------------------------------------------------------------------------------

- 取消原有配置文件.env，通过app-config.js取代，配置更加灵活直观
- 调整插件目录，添加core目录，此目录下的插件需优先注入，确保后续插件可以使用全局配置与方法
- 调整加载路径，此前使用相对路径，比较繁琐；调整后采用绝对路径process.cwd()，配置文件中也请使用绝对路径
- 增加路由登录拦截配置，在middlewares中添加了路由拦截模块，对请求进行拦截
...
- 该目录结构仅作参考
- prisma仅为本人喜欢用的orm框架，如果不使用orm框架或者使用别的orm框架，可以将prisma相关文件和依赖自行删除，再引入别的依赖和文件（2024/6/11 已新增typeorm示例）
...
1. 项目根目录下执行
    ...
    npm run dev
    ```
2. 访问 `http://localhost:3000/client/pub/test/prisma` 即可看到项目启动成功
